...
- 主布局右上角展示全局实时钟表：默认显示数字年月日时分秒，每秒刷新；点击可切换为放大模拟表盘，表盘含 60 个秒刻度、12 个数字、时针、分针、秒针和单独年月日展示；时钟定时器由 `web/core.js` 单例管理，避免页面重渲染时重复创建 `setInterval`。
...
- 新增全局实时钟表：登录后右上角展示数字时间，点击切换为带刻度模拟表盘，作为家庭共用电脑上的时间提示。

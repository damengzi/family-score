...
后续如增强安全性，优先考虑“只在首次初始化时创建管理员，不在每次启动时强制重置密码”。
...
已完成的前端体验增强均为原生 JS/CSS 实现，不新增后端接口、不改数据库结构：
...
维护注意：上述快捷模板和愿望单属于前端辅助能力，不应绕过后端权限、积分规则、任务审核上限或兑换审核规则。若后续要让愿望单跨设备同步，需要新增后端模型、迁移和权限校验。
...
1. 管理员密码不要每次启动都重置为默认密码。
2. 忘记密码功能禁止或限制重置管理员账号。
3. 家长权限边界进一步收紧，尤其是孩子归属转移和兑换审核。
4. 任务审核改为事务内原子处理，避免并发重复加分。
5. 兑换审核时二次校验奖励状态、孩子基准分和账户余额。
6. 给 POST/PATCH/DELETE 增加基础 CSRF/Origin 校验。
7. 增加孩子端“今日成长进度”、家长端“待审核提醒”、管理员端“家庭总览”。

...
func (r *Repository) Migrate(ctx context.Context) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS task_templates (id INTEGER PRIMARY KEY AUTOINCREMENT, family_id INTEGER NOT NULL, task_name VARCHAR(128) NOT NULL, task_type VARCHAR(32) NOT NULL, category VARCHAR(32) NOT NULL, subject VARCHAR(32) NOT NULL DEFAULT 'GENERAL', content VARCHAR(2048) NOT NULL DEFAULT '', question_type VARCHAR(32) NOT NULL DEFAULT 'NONE', answer VARCHAR(512) NOT NULL DEFAULT '', questions TEXT NOT NULL DEFAULT '[]', score_value INTEGER NOT NULL, target_account VARCHAR(32) NOT NULL, due_time VARCHAR(5) NOT NULL DEFAULT '', need_parent_confirm INTEGER NOT NULL DEFAULT 1, daily_limit INTEGER NOT NULL DEFAULT 1, weekly_limit INTEGER NOT NULL DEFAULT 0, enabled INTEGER NOT NULL DEFAULT 1, description VARCHAR(512) NOT NULL DEFAULT '', created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);`,
		`CREATE TABLE IF NOT EXISTS task_instances (id INTEGER PRIMARY KEY AUTOINCREMENT, child_id INTEGER NOT NULL, template_id INTEGER NULL, task_name VARCHAR(128) NOT NULL, task_type VARCHAR(32) NOT NULL, category VARCHAR(32) NOT NULL, subject VARCHAR(32) NOT NULL DEFAULT 'GENERAL', content VARCHAR(2048) NOT NULL DEFAULT '', question_type VARCHAR(32) NOT NULL DEFAULT 'NONE', answer VARCHAR(512) NOT NULL DEFAULT '', questions TEXT NOT NULL DEFAULT '[]', score_value INTEGER NOT NULL, target_account VARCHAR(32) NOT NULL, status VARCHAR(32) NOT NULL DEFAULT 'TODO', submit_note VARCHAR(512) NOT NULL DEFAULT '', audit_note VARCHAR(512) NOT NULL DEFAULT '', task_date DATE NOT NULL, due_at TIMESTAMP NULL, submitted_at TIMESTAMP NULL, audited_at TIMESTAMP NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);`,
		...
		`CREATE TABLE IF NOT EXISTS wishes (id INTEGER PRIMARY KEY AUTOINCREMENT, child_id INTEGER NOT NULL, wish_name VARCHAR(128) NOT NULL, wish_type VARCHAR(32) NOT NULL DEFAULT 'REWARD', expected_score INTEGER NOT NULL DEFAULT 0, expected_star INTEGER NOT NULL DEFAULT 0, reason VARCHAR(512) NOT NULL DEFAULT '', status VARCHAR(32) NOT NULL DEFAULT 'PENDING', audit_note VARCHAR(512) NOT NULL DEFAULT '', created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, audited_at TIMESTAMP NULL, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);`,
		`CREATE TABLE IF NOT EXISTS appeals (id INTEGER PRIMARY KEY AUTOINCREMENT, child_id INTEGER NOT NULL, target_type VARCHAR(32) NOT NULL DEFAULT 'SCORE', target_id INTEGER NOT NULL DEFAULT 0, record_id INTEGER NOT NULL DEFAULT 0, appeal_reason VARCHAR(512) NOT NULL, expected_solution VARCHAR(512) NOT NULL DEFAULT '', status VARCHAR(32) NOT NULL DEFAULT 'PENDING', handle_result VARCHAR(32) NULL, handle_note VARCHAR(512) NOT NULL DEFAULT '', created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, handled_at TIMESTAMP NULL);`,
	...
	for _, table := range []string{"task_templates", "task_instances"} {
		...
		if err := addColumnIfMissing(ctx, r.DB, table, "questions", "TEXT NOT NULL DEFAULT '[]'"); err != nil {
			return err
		}
	}
	if err := addColumnIfMissing(ctx, r.DB, "task_templates", "due_time", "VARCHAR(5) NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	if err := addColumnIfMissing(ctx, r.DB, "task_instances", "due_at", "TIMESTAMP NULL"); err != nil {
		return err
	}
	if err := addColumnIfMissing(ctx, r.DB, "appeals", "target_type", "VARCHAR(32) NOT NULL DEFAULT 'SCORE'"); err != nil {
		return err
	}
	if err := addColumnIfMissing(ctx, r.DB, "appeals", "target_id", "INTEGER NOT NULL DEFAULT 0"); err != nil {
		return err
	}

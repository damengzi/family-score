...
func (s *Service) createRepairTaskForDeduct(ctx context.Context, req protocol.CreateScoreRecordParam) error {
	...
	_, err := s.repo.DB.ExecContext(ctx, `INSERT INTO task_instances(child_id, template_id, task_name, task_type, category, subject, content, question_type, answer, score_value, target_account, task_date) VALUES(?, NULL, ?, 'REPAIR', 'EMOTION', 'MORAL', ?, 'SHORT_ANSWER', ?, ?, 'BASE', DATE('now', 'localtime'))`, req.ChildID, "修复任务："+name, content, "能说出原因、影响和下次做法，并完成补救行动", score)
	return err
}
